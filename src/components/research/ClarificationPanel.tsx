  ...
  return (
    <div className="bg-cyber-800 border border-cyber-600 rounded-xl p-6 md:p-8 shadow-card transition-all">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="mt-6 pt-5 border-t border-cyber-600">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              {assumptions.map((item, idx) => (
                <div
                  key={idx}
                  className="text-xs bg-cyber-750/70 border border-cyber-600 p-3.5 rounded-xl"
                >
                  <div className="font-semibold text-icy-100 flex items-center justify-between">
                  ...
                  <p className="text-icy-300 mt-1.5">{item.reason}</p>
        ...
        <div className="flex items-center justify-end gap-3 pt-6 border-t border-cyber-600">
          <button
            type="button"
            onClick={onSkipToDefine}
            className="px-4 py-2.5 text-xs font-medium text-icy-300 hover:text-icy-100 hover:bg-cyber-750 rounded-lg transition-colors border border-transparent"
          >
