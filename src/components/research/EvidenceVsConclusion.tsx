...
export const EvidenceVsConclusion: React.FC<EvidenceVsConclusionProps> = ({ learn }) => {
  return (
    <div className="bg-midnight-800 border border-midnight-600 rounded-xl p-6 md:p-8 shadow-card transition-all space-y-6">
      <div className="flex items-center justify-between border-b border-midnight-600 pb-5">
        <div>
        ...
        <div className="hidden sm:flex items-center gap-1.5 text-xs text-violet-hover bg-violet-soft px-3 py-1.5 rounded-full border border-violet-accent/30 font-medium">
          <ShieldCheck className="w-3.5 h-3.5 text-violet-hover" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-midnight-750/80 border border-midnight-600 rounded-xl p-5 space-y-3.5">
          <div className="flex items-center gap-2 text-status-success font-bold text-xs uppercase tracking-wider border-b border-midnight-600 pb-3">
            <FileText className="w-4 h-4 text-status-success" />
            <span>1. What the Data Shows (Factual Evidence)</span>
          </div>
          <ul className="space-y-2.5 text-xs text-slate-textSecondary">
        ...
        <div className="bg-midnight-750/80 border border-midnight-600 rounded-xl p-5 space-y-3.5">
          <div className="flex items-center gap-2 text-violet-hover font-bold text-xs uppercase tracking-wider border-b border-midnight-600 pb-3">
            <Compass className="w-4 h-4 text-violet-hover" />
            <span>2. What We Can Reasonably Conclude</span>
          </div>
          <ul className="space-y-2.5 text-xs text-slate-textSecondary">
            {learn.cautiousConclusions.map((item, idx) => (
              <li key={idx} className="flex items-start gap-2.5">
                <span className="w-1.5 h-1.5 rounded-full bg-violet-hover shrink-0 mt-2" />
                <span>{item}</span>
              </li>
            ))}
          </ul>
      ...
      <div className="pt-3">
        <h4 className="text-[11px] font-bold text-slate-textMuted uppercase tracking-wider mb-3.5 flex items-center gap-2">
          ...
          {learn.researchRisks.map((risk, idx) => {
            const severityStyle =
              risk.severity === "high"
                ...
                : "bg-midnight-700 border-midnight-600 text-slate-textSecondary";

            return (
              <div
                key={idx}
                className="p-4 bg-midnight-750/90 border border-midnight-600 rounded-xl space-y-2"
              >
                ...
                <p className="text-[11px] text-slate-textSecondary">{risk.description}</p>
                <div className="text-[11px] text-violet-hover bg-violet-soft p-2.5 rounded-lg border border-violet-accent/25 font-medium">
                  <strong className="text-slate-textPrimary">Mitigation:</strong> {risk.mitigation}
                </div>
              </div>
            );
          })}
