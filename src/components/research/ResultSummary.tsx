...
export const ResultSummary: React.FC<ResultSummaryProps> = ({ result }) => {
  ...
  return (
    <div className="bg-cyber-800 border border-cyber-600 rounded-xl p-6 md:p-8 shadow-card transition-all space-y-6">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-cyber-600 pb-5 gap-2">
        <div>
        ...
        <div className="text-right">
          <span className="text-[11px] bg-cyber-750 text-icy-300 border border-cyber-600 px-3 py-1.5 rounded-lg font-mono">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5">
        {/* Events Count */}
        <div className="bg-cyber-750/90 border border-cyber-600 p-3.5 rounded-xl">
          <div className="flex items-center gap-1.5 text-[11px] font-semibold text-icy-500 mb-1.5">
            <Hash className="w-3.5 h-3.5 text-icy-500" />
            <span>Sample Events</span>
          </div>
          <div className="text-xl font-bold text-icy-100 font-mono">
            {metrics.qualifyingEvents}
          </div>
          <span className="text-[10px] text-icy-500">Qualifying triggers</span>
        </div>
        ...
        <div className="bg-cyber-750/90 border border-cyber-600 p-3.5 rounded-xl">
          ...
          <span className="text-[10px] text-icy-500">Net profitable trades</span>
        </div>

        {/* Mean Net Forward Return */}
        <div className="bg-cyber-750/90 border border-cyber-600 p-3.5 rounded-xl">
          <div className="flex items-center gap-1.5 text-[11px] font-semibold text-icy-500 mb-1.5">
            <TrendingUp className="w-3.5 h-3.5 text-coolTeal-400" />
            <span>Mean Return</span>
          </div>
          <div
            className={`text-xl font-bold font-mono ${
              metrics.meanForwardReturnPct >= 0 ? "text-coolTeal-400" : "text-status-error"
            }`}
          >
            {metrics.meanForwardReturnPct > 0 ? "+" : ""}
            {metrics.meanForwardReturnPct}%
          </div>
          <span className="text-[10px] text-icy-500">
        ...
        <div className="bg-cyber-750/90 border border-cyber-600 p-3.5 rounded-xl">
          <div className="flex items-center gap-1.5 text-[11px] font-semibold text-icy-500 mb-1.5">
            <Scale className="w-3.5 h-3.5 text-icy-500" />
          ...
          <span className="text-[10px] text-icy-500">All-session drift</span>
        </div>

        {/* Edge vs Baseline */}
        <div className="bg-cyber-750/90 border border-cyber-600 p-3.5 rounded-xl">
          <div className="flex items-center gap-1.5 text-[11px] font-semibold text-icy-500 mb-1.5">
            <Award className="w-3.5 h-3.5 text-coolTeal-400" />
          ...
          <span className="text-[10px] text-icy-500">Excess over baseline</span>
        </div>

        {/* Sharpe Ratio */}
        <div className="bg-cyber-750/90 border border-cyber-600 p-3.5 rounded-xl">
          <div className="flex items-center gap-1.5 text-[11px] font-semibold text-icy-500 mb-1.5">
            <Activity className="w-3.5 h-3.5 text-icy-500" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pt-2">
        <div className="lg:col-span-2 bg-cyber-750/80 border border-cyber-600 p-5 rounded-xl">
          <h4 className="text-[11px] font-bold text-icy-500 uppercase tracking-wider mb-4">
            Forward Return Distribution (Net of Costs)
          </h4>
          <div className="h-52 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={metrics.distributionBuckets}>
                <XAxis dataKey="range" tick={{ fontSize: 10, fill: "#6B8EA8" }} stroke="#26446E" />
                <YAxis tick={{ fontSize: 10, fill: "#6B8EA8" }} stroke="#26446E" />
                <Tooltip
                  ...
                  itemStyle={{ color: "#F1FAFE" }}
                />
                <Bar dataKey="count" radius={[4, 4, 0, 0]}>
                  {metrics.distributionBuckets.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={entry.isPositive ? "#0096C7" : "#F59E0B"}
                    />
                  ))}
                </Bar>
              </BarChart>
        ...
        <div className="bg-cyber-750/80 border border-cyber-600 p-5 rounded-xl flex flex-col justify-between space-y-4">
          <div>
            <h4 className="text-[11px] font-bold text-icy-500 uppercase tracking-wider mb-3">
              Distribution Tail Risk
            </h4>
            <div className="space-y-2.5 text-xs">
              <div className="flex items-center justify-between p-3 bg-cyber-800 rounded-lg border border-cyber-600">
                <span className="text-icy-300 flex items-center gap-1.5">
          ...
          {metrics.warnings.length > 0 && (
            <div className="p-3 bg-status-warning/10 border border-status-warning/25 rounded-xl text-[11px] text-status-warning space-y-1.5">
              ...
              <ul className="list-disc pl-4 space-y-0.5 text-icy-300">
